...
for k = 1 : 120
    ...
    fprintf('Loading...  %.2f/100.00\n', (k*100)/120);
end

fclose(fid);
implay(detected_vid_5m);

fid = fopen('ref_5m_1080.txt','r');
formatSpec = '%d';
...
OBJ3 = VideoReader('inapoi_1080.mp4');
...
fid=fopen('inapoi_1080.txt','w');
fid2=fopen('cadre_fara_detectie_inapoi_1080.txt','w');
...
for k = 1 : numar_cadre_depl
    ...
    fprintf('Loading...  %.2f/100.00 ', (k*100)/numar_cadre_depl);
    fprintf('\n');
end
...
fid2 = fopen('cadre_fara_detectie_inapoi_1080.txt','r');
formatSpec = '%d';
...
fid=fopen('distance_inapoi_1080.txt','w');
...
fid = fopen('distance_inapoi_1080.txt','r');
formatSpec = '%f';
...
%afisare
index = 0;
OBJ4 = VideoReader('inapoi_1080.mp4');
...
for k = 1:numar_cadre_depl
 ...
 if isempty(bboxdepl) == 1
     index = index + 1;
     detplusdist = insertObjectAnnotation(detected_vid_depl(:,:,:,k), 'rectangle',pozitiehd, label_no_det(k),'TextBoxOpacity',1,'FontSize',18,'Color','red');
     detected_vid_dist(:,:,:,k) = detplusdist;
    
 else
detplusdist = insertObjectAnnotation(detected_vid_depl(:,:,:,k), 'rectangle', pozitie , label_str(abs(k-index)),'TextBoxOpacity',1,'FontSize',18,'Color','green');
detected_vid_dist(:,:,:,k) = detplusdist;
 end
 fprintf('Loading...  %.2f/100.00\n', (k*100)/numar_cadre_depl);
 
end


implay(detected_vid_dist,50);
